...
public interface VehicleCarRepository extends JpaRepository<VehicleCar, Integer>, JpaSpecificationExecutor<VehicleCar> {
    ...
    @Query("SELECT v FROM VehicleCar v LEFT JOIN FETCH v.projectTag LEFT JOIN FETCH v.carBrand LEFT JOIN FETCH v.carModel WHERE v.user.id = :userId ORDER BY v.createdAt DESC")
